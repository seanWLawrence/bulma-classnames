...
class BulmaClassname {
	...
	formatString(selector: string, value: string = ''): string {
		...
		default:
			if (process.env.NODE_ENV === 'development') {
				// if in development mode, log a simple warning to the console
				console.warn(
	...
	formatClassname(selector: string, value: string | string[] = ''): string {
		...
		// if value if neither of the above, log a warning to the console and return an empty string
		...
		// value is not a string or an array of only strings, return an empty string
		return '';
	}
...
const bc = (classnames: Classnames): string =>
	new BulmaClassname(classnames).result;

// default export so it can be imported like `import bc from 'bulma-classnames'`
export default bc;
